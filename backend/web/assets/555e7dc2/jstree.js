 ...
 */
	 ...
	 */
	 $.jstree.defaults.sort = function (a, b) {
	 	var datas = [this.get_text(a), this.get_text(b)];
	 	var results =[];
	 	$.each( datas, function( key, value) {
	 		results[key] = value.split(' ');
	 		if(results[key][0].length){

	 			//console.log(results[key][0].split('.'));
	 			var chackDelimi =results[key][0].split('.');
	 			var chackWing =results[key][0].split(')');
	 			if($.trim(chackDelimi[0]).length || $.trim(chackWing[0]).length){
	 				var dataChack = results[key][0].concat('.0.0.0');
	 				var a = dataChack.indexOf(".");
					var b = dataChack.indexOf(".",a + 1);
					var c = dataChack.indexOf(".",b + 1);
					var e = dataChack.indexOf(".",c + 1);

	 				results[key] = dataChack.substring(0,e);//dataChack.substr(dataChack.indexOf('.',5));

	 				//console.log(dataChack+' |  = '+datas[key]+ ' | '+dataChack.substr(1, dataChack.indexOf('.')));
				}
			}else{
				results[key] = value;
			}
		...
		//console.log(var streetaddress= addy.substr(0, addy.indexOf(',')); );
		//console.log('3.10.0.00'+' | '+'3.1.9.00.0'+' = '+('3.10.0.00' > '3.1.9.00.0' ? -1 : 1));
		/*var aURL = "3.10".concat('.0.0.0');
		var a = aURL.indexOf(".");
		var b = aURL.indexOf(".",a + 1);
		var c = aURL.indexOf(".",b + 1);
		var e = aURL.indexOf(".",c + 1);*/
			//3.10.0.0 .0.0
			//pos = foo.indexOf(".", pos+1);
			
				//console.log(aURL.substring(0,e).concat(aURL.substring(e+1)));
    			//console.log(foo.replace('.', ''));
			
		if(results[0].length == results[1].length){
			console.log('#1 '+results[0]+' | '+results[1]+' = '+(results[0].length > results[1].length ? -1 : 1)+ ' | ( '+this.get_text(a)+' '+this.get_text(b)+' )');
			return results[0] > results[1] ? 1 : -1;
		}else{
			if(results[0].length > results[1].length){
				return 1;
			...
			//console.log('#2 '+results[0]+' | '+results[1]+' = '+(results[0] > results[1] ? 1 : -1)+ ' | ( '+this.get_text(a)+' '+this.get_text(b)+' )');
			//return -1;
		}
